...
function shallowCopy(obj) {
  const newObj = {};
  const clone = Object.assign(newObj, obj);
  return clone;
}
...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i += 1) {
    Object.entries(objects[i]).forEach((e) => {
      const [key, value] = e;
      if (newObj[key]) {
        newObj[key] += value;
      } else {
...
function makeWord(lettersObject) {
  ...
  for (let i = 0; i < newArr.length; i += 1) {
    for (let j = 0; j < 1; j += 1) {
      for (let k = 0; k < newArr[i][j + 1].length; k += 1) {
        arr[newArr[i][j + 1][k]] = newArr[i][j];
      }
    }
  }
  return arr.join('');
...
const cssSelectorBuilder = {
  ...
  combine(selector1, combinator, selector2) {
    const newObj = Object.create(cssSelectorBuilder);
    newObj.res = `${selector1.res} ${combinator} ${selector2.res}`;
    return newObj;
  },
  stringify() {
    return this.res;
